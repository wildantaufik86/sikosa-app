...
describe("Consultation Service - Psikolog (FULL TEST)", () => {
  ...
  test("CONS-PSI-10 : kirim pesan saat pending", async () => {
    ...
    await expect(sendMessage({ userId: psychologistId, consultationId, message: "hi" })).rejects.toMatchObject({
      statusCode: 403,
    });
  });
  ...
  test("CONS-PSI-12 : kirim pesan kosong", async () => {
    ...
    await expect(sendMessage({ userId: psychologistId, consultationId, message: "" })).rejects.toMatchObject({ statusCode: 400 });
  });
  ...
  test("CONS-PSI-14 : akses chat bukan miliknya", async () => {
    ...
    await expect(sendMessage({ userId: otherPsychologistId, consultationId, message: "hi" })).rejects.toMatchObject({
      statusCode: 403,
    });
  });
