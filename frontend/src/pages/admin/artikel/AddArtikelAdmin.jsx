...
import { AdmincreateArticle } from "../../../utils/api";
...
const AddArtikelAdmin = () => {
  ...
  const handlePostArticle = async (e) => {
    ...
    try {
      ...
      const response = await AdmincreateArticle(formData);
      if (response.error) {
      ...
      <form onSubmit={handlePostArticle}>
        ...
        <div className="flex justify-between items-center border-b border-black pb-2 mb-4">
          ...
          <div className="space-x-2">
            ...
            <button type="submit" className="px-4 py-1 bg-[#35A7FF] text-sm text-white rounded-lg">
              Post
            </button>
          </div>
        </div>
        ...
        <div className="mb-6">
          <label className="flex items-center mb-2">
            <AiFillStar className="text-[10px] text-red-500 mr-2" /> {/* Red star icon */}
